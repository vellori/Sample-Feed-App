...
public extension UseCases {
    enum ArticleFeedUseCases {
        ...
        case landing
        @ViewBuilder public func view<Detail: View>(detailViewProvider: @escaping ArticleDetailViewProvider<Detail>) -> some View {
            let viewModel = ArticleFeedViewModel(articleFeedService: ArticleFeedService(urlDownloader: URLDownloaderService(), decodingService: ArticleFeedDecodingService()))
